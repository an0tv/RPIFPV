#!/usr/bin/env bash
#
# install.sh — one-shot installer for the OpenIPC Pi Zero 2 W.
# Run as root ON THE PI after copying this repo over (e.g. /opt/openipc).
#
#   sudo bash setup/install.sh
#
# What it does:
#   1. installs packages (go2rtc binary, ffmpeg, v4l-utils, network-manager)
#   2. detects the camera and renders /etc/openipc/go2rtc.yaml
...
# ---- editable -------------------------------------------------------------
INSTALL_DIR="${INSTALL_DIR:-/opt/openipc}"
RUN_USER="${RUN_USER:-openipc}"
GO2RTC_VERSION="${GO2RTC_VERSION:-latest}"          # or pin e.g. v1.9.13
# URL-encoded special char trick for # in systemd is avoided by using -config
# ---------------------------------------------------------------------------
...
apt-get install -y --no-install-recommends \
  ffmpeg v4l-utils network-manager dnsmasq ca-certificates curl unzip python3
...
mkdir -p "$INSTALL_DIR/web" /etc/openipc /var/log/openipc

cp -r "$(dirname "$0")/web/." "$INSTALL_DIR/web/"
cp "$(dirname "$0")/wifi/netd.sh" "$INSTALL_DIR/netd.sh"
chmod +x "$INSTALL_DIR/netd.sh" "$INSTALL_DIR/web/portal.py"
...
# ---- default wifi config --------------------------------------------------
if [ ! -f /etc/openipc/wifi.json ]; then
  cp "$(dirname "$0")/wifi/wifi.default.json" /etc/openipc/wifi.json
fi
chmod 600 /etc/openipc/wifi.json

# ---- systemd --------------------------------------------------------------
echo "==> installing systemd units"
cp "$(dirname "$0")/systemd/"*.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable openipc-net.service openipc-camera.service openipc-portal.service

echo
echo "==> done. First boot: the Pi tries its saved network for 15s,"
echo "    otherwise starts AP '$(sed -n 's/.*"ap_ssid": *"\([^"]*\)".*/\1/p' /etc/openipc/wifi.json || echo openipc-cam)'."
echo
...
echo "      systemctl start openipc-net openipc-camera openipc-portal"
